...
- [ ] **RLS 確認**: anon クライアントでキャッシュするには `works` / `work_variants` / `work_offers` / `work_series` / `production_projects` / `production_outputs` が anon SELECT 可能であること（Supabase MCP, read-only で確認）。
