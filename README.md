...
このリポジトリは現在、Gallery(Next.js) に IMAGINE を統合し、**単一アプリ・単一ドメイン（`whatif-ep.xyz`）** へ再設計するリニューアルを進行中（ブランチ `renewal/single-app`）。現行 README は本番運用中の構成説明。

正本ドキュメント（この順で読む）:

1. [docs/PRODUCT_ROADMAP.md](docs/PRODUCT_ROADMAP.md) — プロダクト全体の正本（4層ラダー / 価格 / The Club 格下げ）
2. [docs/ARCHITECTURE_OVERVIEW.md](docs/ARCHITECTURE_OVERVIEW.md) — 2リポジトリ横断地図
3. [docs/CONSOLIDATION_PLAN.md](docs/CONSOLIDATION_PLAN.md) — 統合の技術設計・実施ログ
4. [docs/CUTOVER_MILESTONES.md](docs/CUTOVER_MILESTONES.md) — 停止までの実行順・完了条件（M1-M6）
5. [docs/NEXT_SESSION_HANDOFF.md](docs/NEXT_SESSION_HANDOFF.md) — 次回再開の短縮版（現在 M3）
